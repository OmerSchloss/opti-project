...
""" Compute an upper bound based on a gready nearest neighbour seach. """
def computeUpperBound(graph):
    ...
    try:
        ...
        while len(graph.nodes()) > 1:
            ...
            current_node = nn[1]
        upper_bound += list(filter(lambda edge: edge[1] == current_node, last_edges))[0][2]["weight"]
    except:
        upper_bound = float("inf")
    return upper_bound

"""Compute a lower bound for a tree node with the Held & Karp method."""
def computeLowerBound(graph, ub, relax_solved, l=0):
    ...
    for m in range(M):
        ...
        t1 = real_cost / (2 * n)
        t = ( t1 * (m - 1) * (2 * M - 5) / (2 * M - 2)- t1 * (m - 2) + 0.5 * t1 * (m - 1) * (m - 2) / ((M - 1) * (M - 2)))
...
"""Return if the 1-tree is a cycle or not."""
def isCycle(tree):
...
Each node of the tree has four parameter :
...
def solve(problem, timeLimit):
    ...
    # Tsplib95 add a loop over each node by default so we have to remove all these loops.
    ...
    node_to_explore = []
    ub = computeUpperBound(graph.copy())
    print("The first upper bound is computed with a greedy nearest-neighbour algorithm:", ub)
    best_path = None
    nodes_explored = 0
    relax_solved = 0
    ...
    root["lb"], root["path"], root["l"], relax_solved = computeLowerBound(
        root["graph"].copy(), ub, relax_solved
    )
    if isCycle(root["path"]) and root["lb"] < ub:
        ub = root["lb"]
        best_path = root["path"]

    print( "Nodes explored : 0 | Resolved relaxations :",relax_solved,"| Upper bound :",format(ub, ".2f"),"| Lower bound :",
        format(root["lb"], ".2f"), "| Gap :",format(100 * abs(ub - root["lb"]) / ub, ".2f"),"%")
    ...
    while node_to_explore:
        ...
        if time.time() - start > timeLimit:
            return ("Max time reached", None, None, timeLimit, None, None)

        # Take the node on the top of the pile
        node = node_to_explore.pop()
        nodes_explored += 1
        if nodes_explored % 10 == 0:
            print("Nodes explored :",nodes_explored, "| Resolved relaxations :", relax_solved,"| Upper bound :", format(ub, ".2f"),
                "| Current lower bound :", format(node["lb"], ".2f"),"| Gap :", format(100 * abs(ub - node["lb"]) / ub, ".2f"),"%")
        ...
        for edge in range(len(edges_to_remove)):
            ...
            child_nodes[edge] = { "graph": node["graph"].copy(), "level": node["level"] + 1}
            child_nodes[edge]["graph"].remove_edge(edges_to_remove[edge][0], edges_to_remove[edge][1])
            ...
            if nx.is_connected(child_nodes[edge]["graph"]):
                ...
                local_ub = computeUpperBound(child_nodes[edge]["graph"].copy())
                ub = min(ub, local_ub)
                ...
                ( child_nodes[edge]["lb"], child_nodes[edge]["path"], child_nodes[edge]["l"], relax_solved) = computeLowerBound(child_nodes[edge]["graph"].copy(), local_ub, relax_solved, node["l"])

                # Test if it's a new upper bound.
                if (isCycle(child_nodes[edge]["path"]) and child_nodes[edge]["lb"] < ub):
    ...
    return status, ub, cyclePath, stop - start, nodes_explored, relax_solved


def branchandbound(filename, timeLimit):
    ...
    (status, optimal_value, optimal_path, solving_time, nodes_explored, relax_solved,) = solve(problem, timeLimit)
